...
This repository now includes a full Java port of the packet analyzer and DPI engine under `src/main/java`. The Java implementation covers:

- `PacketAnalyzerCli`: packet-by-packet inspection
- `SimpleDpiCli`: single-threaded DPI filtering
- `MultithreadedDpiCli`: multi-threaded DPI pipeline
...
```bash
java -cp target/classes com.packetanalyzer.dpi.cli.PacketAnalyzerCli test_dpi.pcap 5
```

Single-threaded DPI:

```bash
java -cp target/classes com.packetanalyzer.dpi.cli.SimpleDpiCli test_dpi.pcap filtered_simple.pcap --block-domain youtube
```

Multi-threaded DPI:

```bash
java -cp target/classes com.packetanalyzer.dpi.cli.MultithreadedDpiCli test_dpi.pcap filtered_mt.pcap --block-app YouTube --lbs 2 --fps 2
```

Maven metadata is also included in `pom.xml` for standard Java project structure and IDE import.
...
|---------|------|----------|
| Simple (Single-threaded) | `com.packetanalyzer.dpi.cli.SimpleDpiCli` | Learning, small captures |
| Multi-threaded | `com.packetanalyzer.dpi.cli.MultithreadedDpiCli` | Production, large captures |
...
```
packet_analyzer/
├── pom.xml                     # Java project metadata
├── src/main/java/com/packetanalyzer/dpi/
│   ├── cli/                   # CLI entry points
│   │   ├── PacketAnalyzerCli.java
│   │   ├── SimpleDpiCli.java
│   │   └── MultithreadedDpiCli.java
│   ├── io/                    # PCAP read/write
│   ├── parse/                 # Ethernet/IP/TCP/UDP parsing
│   ├── inspect/               # TLS SNI / HTTP Host / DNS extraction
│   ├── engine/                # Simple and multi-threaded DPI engines
│   ├── rules/                 # Blocking rules
│   ├── model/                 # FiveTuple, AppType, packet models
│   └── util/                  # Protocol and classification helpers
...
Let's trace a single packet through `com.packetanalyzer.dpi.engine.SimpleDpiEngine`:
...
The multi-threaded version (`MultithreadedDpiEngine.java`) adds **parallelism** for high performance:
...
### `PcapReader.java` / `PcapWriter.java`
...
### `AppType.java`, `FiveTuple.java`, and shared models
...
- No external runtime libraries needed
...
**Optional Maven metadata import/build:**
```bash
mvn package
```
...
**Basic usage:**
```bash
java -cp target/classes com.packetanalyzer.dpi.cli.MultithreadedDpiCli test_dpi.pcap output.pcap
```
...
```bash
java -cp target/classes com.packetanalyzer.dpi.cli.MultithreadedDpiCli input.pcap output.pcap --lbs 4 --fps 4
# Creates 4 LB threads × 4 FP threads = 16 processing threads
```

**Single-threaded DPI:**
```bash
java -cp target/classes com.packetanalyzer.dpi.cli.SimpleDpiCli test_dpi.pcap output_simple.pcap --block-domain youtube
```

**Packet analyzer view:**
```bash
java -cp target/classes com.packetanalyzer.dpi.cli.PacketAnalyzerCli test_dpi.pcap 10
```
...
If you have questions about any part of this project, the Java code follows the same flow described in this document. Start with [`SimpleDpiEngine.java`](/Users/aashishkumar/Packet_analyzer/src/main/java/com/packetanalyzer/dpi/engine/SimpleDpiEngine.java) to understand the core pipeline, then move to [`MultithreadedDpiEngine.java`](/Users/aashishkumar/Packet_analyzer/src/main/java/com/packetanalyzer/dpi/engine/MultithreadedDpiEngine.java) to see how parallelism is added.
